# JOB C SPECIFICATION — Replace Paid API Dependencies with Offline Engines
**Consumed by:** JOB_C_PROMPT.txt agent team  
**Touches:** chatbot/router.py · rag/qa_engine.py · modules/qa/router.py

---

## BACKGROUND — WHY THIS JOB EXISTS

Two features in the platform call the Anthropic Claude paid API:

1. **Master Chatbot** (`app/modules/chatbot/router.py`) — calls `anthropic.Anthropic()` with the model `claude-sonnet-4-5`. Without a paid API key, the entire chatbot widget crashes with HTTP 503.

2. **Document Q&A Engine** (`app/rag/qa_engine.py`) — also calls the Anthropic API to generate answers from retrieved document chunks.

Both must be replaced with fully offline implementations. This is not a downgrade — for a thesis demonstration project with a fixed dataset of Zimbabwean insurance documents, keyword-based retrieval is academically justifiable (Jones, 1972; TF-IDF basis) and produces reliable results on domain-specific text.

---

## TASK 1 — Rewrite `app/modules/chatbot/router.py`

Replace the entire file with this offline rule-based implementation:
...
## TASK 2 — Rewrite `app/rag/qa_engine.py`

Replace the entire file with the offline implementation:

```python
"""
app/rag/qa_engine.py
=====================
Offline keyword-based document Q&A engine.
...
class QAEngine:
    ...
    def __init__(self, vector_store: Any = None, anthropic_api_key: str = "") -> None:
        # Kept for API compatibility only — not used in offline mode
    ...
    def _score_sentences(
        ...
        for idx, sentence in enumerate(sentences):
            ...
            if score > 0:
                score += len(stripped) / 2000.0  # length bonus
...
## TASK 3 — Update `app/modules/qa/router.py`

Open the file and find the endpoint that calls `QAEngine.answer()`.  
Update it to:
1. Fetch the document's extracted text from the database
2. Pass it as `document_text=` to `engine.answer()`
3. Remove any imports of `anthropic`, `VectorStore`, or `indexing_pipeline` that are only used for the API call
...
doc_title = getattr(doc, "title", None) or getattr(doc, "filename", "") or f"Document #{document_id}"
...
# V1 — IPEC intent
RESP=$(curl -s -X POST http://127.0.0.1:8000/api/chatbot/message \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"message":"What is IPEC?"}')
echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print('PASS' if 'IPEC' in d['response'] else 'FAIL')"

# V2 — CSP module intent
RESP=$(curl -s -X POST http://127.0.0.1:8000/api/chatbot/message \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"message":"Tell me about the CSP module and WCS score"}')
echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print('PASS' if 'WCS' in d['response'] else 'FAIL')"

# V3 — Fallback for unknown query
RESP=$(curl -s -X POST http://127.0.0.1:8000/api/chatbot/message \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"message":"xyzzy nonsense query 12345"}')
echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print('PASS: fallback works' if d['response'] else 'FAIL')"

# V4 — No anthropic import anywhere
python3 -c "import anthropic" 2>&1 | grep -q "No module" \
  && echo "PASS: anthropic not installed" \
  || grep -rn "import anthropic" app/ --include="*.py" | grep -v "__pycache__" \
  && echo "FAIL: anthropic still imported" || echo "PASS: no anthropic imports"

# V5 — QA engine imports cleanly
python3 -c "from app.rag.qa_engine import QAEngine; e=QAEngine(); print('PASS: QAEngine offline')"

# V6 — Chatbot imports cleanly
python3 -c "from app.modules.chatbot.router import router; print('PASS: chatbot router OK')"
